...
class InputHandler {
    ...
    if (!this._interacting) {
      if (p.mouseIsPressed) {
        if (!this._startedClick) {
          ...
          this._mouseStartX = p.mouseX
          this._mouseStartY = p.mouseY
          this._wasClicking = false
          this._clickHoldCounter = 0
        } else {
        ...
        if (this._startedClick) {
          ...
          this._mouseEndX = p.mouseX
          this._mouseEndY = p.mouseY
  ...
  isMouseOver(clickArea) {
    ...
    return p.mouseX > 0 && p.mouseY > 0 &&
      ...
      p.mouseX >= clickArea.x &&
      p.mouseX < clickArea.x + clickArea.w &&
      p.mouseY >= clickArea.y &&
      p.mouseY < clickArea.y + clickArea.w
  }
}
