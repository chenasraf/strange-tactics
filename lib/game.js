...
function setup() {
	...
	let cols = floor(w / gridSize),
		rows = floor(h / gridSize)

	tiles = new Array(cols)
	party = new Array(partySize)

	for (let i = 0; i < party.length; i++) {
		party[i] = new Character(floor(random(cols)), floor(random(rows)), chars[i], hoverChars[i])
	}

	for (let i = 0; i < tiles.length; i++) {
		tiles[i] = new Array(rows)
		for (let j = 0; j < tiles[i].length; j++) {
			tiles[i][j] = new Tile(i, j, gridSize, grassTile, grassHoverTile)
		}
	}
}
